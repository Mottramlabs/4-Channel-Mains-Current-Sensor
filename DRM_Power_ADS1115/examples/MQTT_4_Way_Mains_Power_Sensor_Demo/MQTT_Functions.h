...
void callback(char* topic, byte* payload, unsigned int length_1) {
  ...
  if (Message_Arrived.indexOf(My_MAC) >= 0 || Message_Arrived.indexOf(Broadcast_All) >= 0) {
    ...
    // tones
    if ((Message_Arrived.indexOf("/Tone=") >= 0) ) {
      Message_Arrived.replace("/Tone=", "");  // Remove the command string characters

      if (Message_Arrived == "Chirp") {
        Mytone.Chirp(Speaker);
      } // end

      if (Message_Arrived == "Bleep") {
        Mytone.Bleep(Speaker);
      } // end

      if (Message_Arrived == "Tone_Down") {
        Mytone.Tone_Down(Speaker);
      } // end

      if (Message_Arrived == "Tone_Up") {
        Mytone.Tone_Up(Speaker);
      } // end

      if (Message_Arrived == "Ringer") {
        Mytone.Ringer(Speaker);
      } // end

      if (Message_Arrived == "Imperial_March") {
        Mytone.Imperial_March(Speaker);
      } // end

      if (Message_Arrived == "Close_Encounters") {
        Mytone.Close_Encounters(Speaker);
      } // end

      if (Message_Arrived == "Harry_Potter") {
        Mytone.Harry_Potter(Speaker);
      } // end
